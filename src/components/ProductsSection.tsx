      ...
      <div className="mx-auto max-w-6xl px-5 md:px-10">
        ...
        <div className="mt-10 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {products.map((p) => (
            <article
              key={p.title}
              className="group flex flex-col overflow-hidden rounded-2xl border-2 border-neutral-900 bg-white shadow-[5px_5px_0_0_#1c1917] transition-transform duration-200 hover:-translate-x-[2px] hover:-translate-y-[2px] hover:shadow-[8px_8px_0_0_#1c1917]"
            >
              {/* Imagem */}
              <div className="relative flex items-center justify-center bg-white p-5">
                <span className="absolute right-3 top-3 rotate-3 rounded-md border-2 border-neutral-900 bg-[#4ADE80] px-2 py-1 font-hero text-[11px] font-black uppercase leading-none text-neutral-900">
                  Oferta
                </span>
                <img
                  src={p.image}
              ...
              <div className="flex flex-1 flex-col border-t-2 border-neutral-900 bg-[#F1E9D9] p-4">
                ...
                <h3 className="mt-1 font-hero text-lg font-black uppercase leading-tight text-neutral-900">
                  {p.title}
                </h3>
                <div className="mt-3 flex items-baseline gap-2">
                  <span className="font-sans text-xs text-neutral-500 line-through">{p.oldPrice}</span>
                  <span className="font-sans text-lg font-extrabold text-neutral-900">{p.price}</span>
                </div>
                <button
                  type="button"
                  className="mt-4 w-full rounded-xl border-2 border-neutral-900 bg-[#4ADE80] py-2.5 font-sans text-sm font-extrabold uppercase tracking-wide text-neutral-900 shadow-[3px_3px_0_0_#1c1917] transition-all duration-150 hover:translate-x-[1px] hover:translate-y-[1px] hover:shadow-[2px_2px_0_0_#1c1917] active:translate-x-[3px] active:translate-y-[3px] active:shadow-none"
                >
                  Comprar
                </button>
              </div>
